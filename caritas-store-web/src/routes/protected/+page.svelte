...
<main>
	<h1 class="visually-hidden">Caritas</h1>

	<div class="px-4 py-5 my-5 text-center">
		<img
			class="d-block mx-auto mb-4"
			src="https://www.fiopsd.org/wp-content/uploads/2012/09/caritas-copia.jpg"
			alt=""
			height="150"
		/>
		
                {#if $currentUser?.role === 'admin'}
		        <h1 class="display-5 fw-bold">Amministratore</h1>
                {:else}
		        <h1 class="display-5 fw-bold">{$currentUser?.user}</h1>
                {/if}
